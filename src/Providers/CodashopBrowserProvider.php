...
final class CodashopBrowserProvider implements ProviderInterface
{
    ...
    public function lookup(array $game, string $playerId, ?string $zoneId = null): LookupResult
    {
        ...
        $script = (string) ($config['script'] ?? ($this->projectRoot . '/scripts/codashop-browser-lookup.mjs'));
        ...
        if (!is_file($script) || !function_exists('proc_open')) {
            return LookupResult::failure(
                ...
                'Browser-assisted Codashop lookup requires Node.js and the bundled browser helper.',
                $game['code'] ?? null,
                $playerId,
                $zoneId,
                $this->key(),
        ...
        $payload = [
            'player_id' => $playerId,
            'page_url' => (string) ($config['pageUrl'] ?? 'https://www.codashop.com/en-my/free-fire'),
            'timeout_ms' => $timeout * 1000,
            'profile_dir' => (string) ($config['profileDir'] ?? ($this->projectRoot . '/template/storage/codashop-browser-profile')),
            'debug_port' => (int) ($config['debugPort'] ?? 9223),
        ];
        ...
        try {
            ...
            return LookupResult::failure(
                ResultCode::PROVIDER_NOT_CONFIGURED,
                'Browser-assisted Codashop lookup is unavailable in this hosting environment.',
                $game['code'] ?? null,
                $playerId,
                $zoneId,
                $this->key(),
        ...
        if (!is_array($data)) {
            return LookupResult::failure(
                ...
                'Browser-assisted Codashop helper returned an invalid response.',
                $game['code'] ?? null,
                $playerId,
                $zoneId,
                $this->key(),
                [
                    ...
                    'stderr' => $this->debug ? trim($result['stderr']) : null,
                ],
            );
        }
        ...
        return LookupResult::failure(
            $code,
            trim((string) ($data['message'] ?? 'Browser-assisted Codashop lookup failed.')),
            $game['code'] ?? null,
            $playerId,
            $zoneId,
            $this->key(),
